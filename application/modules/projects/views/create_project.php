		...
		<div class="col-md-8 col-md-offset-2">
			...
			<?=$this->session->flashdata('form_error') ?>
				<div class="row">
					...
					<div class="col-md-6">
						<div class="form-group">
							<label><?=lang('project_title')?> <span class="text-danger">*</span><span id="already_projecttitle" style="display: none;color:red;">Project Title Already Exists</span></label>
							<input type="text" class="form-control" placeholder="<?=lang('project_title')?>" name="project_title" value="<?=set_value('project_title')?>" id="check_projectname">
						</div>	
					</div>	
				</div>	
				...
				<div class="row">
					...
					<div class="col-md-6">
						<div class="form-group">
							<label><?=lang('items')?> <span class="text-danger">*</span></label>
							<!-- Build your select: -->
							<select class="select2-option" multiple="multiple" required style="width:100%;" name="items[]" > 
								<optgroup label="<?=lang('items')?>"> 
								<?php 
								$all_items = $this->db->get_where('items_saved',array('deleted'=>'No'))->result_array();
								foreach ($all_items as $item ) { ?>
								<option value="<?php echo $item['item_id']; ?>"><?php echo ucfirst($item['item_name']); ?></option>
								<?php } ?>	
								</optgroup> 
							</select>
						</div>
